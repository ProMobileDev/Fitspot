...
import {
  View,Text,Image,TouchableWithoutFeedback,Modal
} from 'react-native'
...
var menuLeftPadding = (270 * (screenDimenssion.width / 414));
var menuRightPadding = (10 * (screenDimenssion.width / 414));
        ...
        return (
          <View style={{flex:1}}>
            <Modal animationType='slide' visible={this.state.modalVisible} onRequestClose={() => this._setModalVisible(false)} supportedOrientations={['portrait']}>
              <AddAvailability chosenDate={this.state.selectedDate} onClosePress={() => this.setState({modalVisible: false})} onCreatePress={this.props.onCreateAvailabilityPress} onDeletePress={this.props.onDeleteAvailabilityPress} onEditPress={this.props.onEditAvailabilityPress} currentAvailability={null}/>
            </Modal>
            <TouchableOpacity style={{position:'absolute',right: 25,top: 32, zIndex:3}} onPress={() => this.setState({modalVisible: true})} >
              <Image style={styles.image} source={require('@images/add-icon.png')} />
            </TouchableOpacity>
          ...
          <MonthlyCalendar onDeleteAvailabilityPress={ this.props.onDeleteAvailabilityPress}
            ...
            createStatus={this.props.createStatus} deleteStatus={this.props.deleteStatus}
             appointmentItems={this.props.sessionItems} onChosenDate={(date) => this.chosenDate(date)} />
          </View>
          </View>
        )
      }else{
      return (
        ...
        <View style={styles.container}>
          <View style={{flexDirection:'row', position:'absolute', top:5,zIndex:5}}>
            ...
            <TouchableOpacity style={{paddingLeft:menuLeftPadding,marginRight:menuRightPadding}} onPress={ this.goToWorkoutSession }>
