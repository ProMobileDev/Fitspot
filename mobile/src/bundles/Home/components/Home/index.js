...
import WorkoutRequestPopup from '@components/PopupWorkoutRequest/modalPopup'
...
class Home extends React.Component {
  ...
  constructor(props){
    ...
    this.state = {
      ...
      workoutRequestModalVisible:false,
      workoutConfirmModalVisible:false,
      workoutRequestDetailMobileVisible:false,
      isVisible: false,
  ...
  componentWillReceiveProps(nextProps){
      ...
      if(this.state.pendingWorkouts.length > 0){
        this.setState({workoutRequestModalVisible: true})
      }
  ...
  setWorkoutRequestModalVisible(visible){
    this.setState({workoutRequestModalVisible: visible})
  }

  setworkoutConfirmModalVisible(){
    this.setState({workoutConfirmModalVisible: visible})
  }
  setworkoutRequestDetailMobileVisible(){
    this.setState({workoutRequestDetailMobileVisible: visible})
  }
...
/* ------------------------------ Upcoming WorkOutSession------------------------ */
  renderCustomerWithWorkOut(){
    ...
    if(typeof nextWorkout == "undefined"){
      ...
      <View style={styles.clientContainer}>
        <View style={{backgroundColor:DEFAULT_GREEN_COLOR, height:150, alignSelf:'stretch'}}>
          <Text style={{marginTop: 35, alignItems:'center', justifyContent:'center', textAlign :'center', color:'white', fontSize:14, fontFamily:FONT_DAYTONA_BOLD}}>There`s a new quick survey for your team!</Text>
          <Button buttonStyle={styles.bookSurveyButton} buttonTextStyle={styles.bookSurveyButtonText}>Take Quick Survey</Button>
        </View>
          
        <Image source={require('../../assets/newHeader.jpg')} style={styles.imageContainer}>
  ...
  //--------testing popup----------
  showPopUp(){
    this.setGiftModalVisible(true);
  }

  renderTrainer(){
    return (
      <View style={styles.clientContainer}>
        <PopupModal visible={this.state.workoutRequestModalVisible} onDismiss={ () => this.setWorkoutRequestModalVisible(false)}>
            <WorkoutRequestPopup />
        </PopupModal>
          ...
          <View style={{paddingBottom:20,backgroundColor:'rgba(255,255,255,.2)',flexDirection:'row',justifyContent:'space-around', width:Dimensions.get('window').width, marginBottom: 20}}>
            <View>
              ...
              <Text style={{color:'white',textAlign:'center', fontFamily: FONT_DAYTONA_BOLD, fontWeight:'bold',fontSize: 10}}>WORKOUTS</Text>
            </View>
            <View>
              ...
              <Text style={{color:'white',textAlign:'center', fontFamily: FONT_DAYTONA_BOLD, fontWeight:'bold',fontSize: 10}}>EARNED</Text>
            </View>
            <View>
              ...
              <Text style={{color:'white',textAlign:'center', fontFamily: FONT_DAYTONA_BOLD, fontWeight:'bold',fontSize: 10}}>CLIENTS</Text>
            </View>
          </View>
        </Image>
        ...
        { (this.state.pendingWorkouts.length + this.state.upcomingWorkouts.length) === 0 ?
          <View style={{ alignItems: 'center',alignSelf:'stretch'}}>
            ...
            <Button buttonStyle={[styles.registerButton,{marginBottom: 100}]} buttonTextStyle={styles.registerButtonText} onPress={ this.props.loggedIn ? ()=> this.showPopUp() : () => alert('Create an Account First to set Availability')}>Set Your Availability!</Button>
          </View>
          :
