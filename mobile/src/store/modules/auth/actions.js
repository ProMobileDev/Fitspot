...
export const AUTH_GET_TOKEN = `${NAMESPACE}/AUTH_GET_TOKEN`;
...
export function getAuthTokenSuccess(token){
  return {type: AUTH_GET_TOKEN, token, receivedAt: Date.now()}
}
    ...
    return ApiUtils.get('auth/token')
      .then(([response, jsonBody]) => {
        if (response.status == 200) {
          ...
          dispatch(getAuthTokenSuccess(jsonBody.token))
        }
      }).catch(err => {
...
export function updateUserAvatar(customer, file) {
  return function(dispatch) {
    var fileInfo = {};
    if(customer.hasOwnProperty("avatarSource")){
      fileInfo = { uri: customer.avatarSource.replace('file://', '') };
    }else{
      fileInfo = file;
    }
